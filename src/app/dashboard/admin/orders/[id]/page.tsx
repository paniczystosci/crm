  ...
  return (
    <div className="max-w-5xl mx-auto space-y-10">
      <div className="flex justify-between items-start">
        ...
        <div className={`px-6 py-2 rounded-2xl text-sm font-medium ${/* status color logic */ ''}`}>
          {order.status}
        </div>
      </div>

      <div className="grid lg:grid-cols-2 gap-8">
        {/* Информация о заказе + редактирование */}
        <div className="card p-8 space-y-8">
          <h2 className="text-2xl font-semibold">Данные заказа</h2>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateOrder('planned_date', e.target.value)}
                  className="w-full px-4 py-3 rounded-2xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900"
                />
              </div>
              <div>
                ...
                <input
                  type="time"
                  value={order.planned_time || ''}
                  onChange={(e) => updateOrder('planned_time', e.target.value)}
                  className="w-full px-4 py-3 rounded-2xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900"
                />
              </div>
            </div>
